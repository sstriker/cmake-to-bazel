...
func Emit(m *toolchain.Model, cfg Config) (*Bundle, error) {
	...
	cfgBzl, err := emitConfigBzl(m, cfg)
	if err != nil {
		return nil, err
	}
...
func emitConfigBzl(m *toolchain.Model, cfg Config) ([]byte, error) {
	var buf bytes.Buffer
	buf.WriteString("# Generated by derive-toolchain. DO NOT EDIT.\n")
	buf.WriteString("\n")
	...
	// compile_flags / dbg_compile_flags / opt_compile_flags
	// We collapse to "compile_flags" (always-on) for now and handle
	// per-build-type flag sets via the variant matrix in a follow-up.
	...
	emitStringList(&buf, "opt_compile_flags", []string{}, "        ")
	emitStringList(&buf, "opt_link_flags", []string{}, "        ")
	emitStringList(&buf, "dbg_compile_flags", []string{}, "        ")
