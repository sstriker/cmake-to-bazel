...
| M5b | 🔧 | Fidelity gate: builds a sample cmake project two ways (cmake reference vs convert-element + bazel) and asserts the resulting libraries' `nm --defined-only` symbol sets match. Catches converter bugs where translation units get dropped or compile flags drift enough to change exports. Without this, "converts cleanly" just means "translation parses cleanly" — a far weaker claim. See `docs/m5b-fidelity-plan.md`. Symbol + behavioral library + plan ship; e2e gate against hello-world wired into CI's bazel-e2e job. | 0.75 | `make e2e-fidelity` passes against hello-world (symbol-tier); the same harness extends to fmt / spdlog as additional fixtures. |
| Toolchain | 🔧 | Derive Bazel `cc_toolchain` / `platform` / `constraint_value` rules by probing cmake configure across a variant matrix (Debug/Release/RelWithDebInfo/MinSizeRel, optionally cross-compile). Replaces hand-written toolchain definitions in M5+M3b deployments with auto-generated ones, ensuring host-toolchain identity matches between cmake and Bazel. See `docs/toolchain-derivation-plan.md`. Step 1 (extract+emit+CLI) ships; step 2 (live-cmake variant probe + per-build-type flag diff) queued. | 1.5 | `derive-toolchain` emits a `cc_toolchain_config` whose downstream `bazel build //:smoke` produces a binary identical to what the same project's cmake build would. |
