...
| Toolchain | ⏳ | Derive Bazel `cc_toolchain` / `platform` / `constraint_value` rules by probing cmake configure across a variant matrix (Debug/Release/RelWithDebInfo/MinSizeRel, optionally cross-compile). Replaces hand-written toolchain definitions in M5+M3b deployments with auto-generated ones, ensuring host-toolchain identity matches between cmake and Bazel. See `docs/toolchain-derivation-plan.md`. | 1.5 | `derive-toolchain` emits a `cc_toolchain_config` whose downstream `bazel build //:smoke` produces a binary identical to what the same project's cmake build would. |
