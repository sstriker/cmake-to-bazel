...
| M3d | ⏳ | BuildStream source CAS integration. FreeDesktop SDK is already a BuildStream project; `bst source push/pull` content-addresses each element's source tree in the project's existing CAS. The orchestrator looks up source by `Source.unique_key()` digest and references it in the action's input root by digest. No orchestrator-side fetching, no source-tree hashing — sources flow through the same CAS the action inputs do. | 1 | An FDSDK subset converts with `--source-cas=grpc://...` against a CAS already populated by `bst source push`; orchestrator never invokes git/tar/curl. |
